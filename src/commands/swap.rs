...
pub fn handle_swap(args: SwapArgs) -> Result<()> {
    ...
    let accounts = match args.direction {
        Direction::In => derive_swap_in_accounts(authority, args.nft_mint, args.fungible_mint),
        Direction::Out => derive_swap_out_accounts(authority, args.nft_mint, args.fungible_mint),
    };
...
fn derive_swap_out_accounts(
    ...
    .0;
    let nft_token_account_source = get_associated_token_address(&escrow_owner, &nft_mint);
    ...
    let fungible_token_account_destination =
        get_associated_token_address(&escrow_owner, &fungible_mint);
